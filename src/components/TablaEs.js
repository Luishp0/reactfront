...
const TablaEs = () => {
    ...
    const getDesaparecidos = async () => {
        const res = await axios.get('http://localhost:8000/desaparecido/activo')
        setDesaparecido(res.data)
        
        
    }
      //para desactivar el desaparecido como dar lo de baja/////////////
    const disableDesaparecido = async (id) => {      
        await axios.put(`http://localhost:8000/desaparecido/desactivar/${id}`)        
        getDesaparecidos()
    }
          ...
          {desaparecidoFiltrado.length > 0 ? (
            desaparecidoFiltrado.map((desaparecido) => (
              ...
              <tr className='tr-lista' key={desaparecido.iddesaparecido}>
                    ...
                    <td>
                        <Link to={`/actualizar/${desaparecido.iddesaparecido}/${userId}`} className='span-tabla'>
                            <GrDocumentUser style={{ fontSize: '25px' }}/>
                        </Link>
                    </td>
                    <td>
                        <button onClick={() => disableDesaparecido(desaparecido.iddesaparecido)} className='span-tabla'>
                            <MdOutlineFolderDelete style={{ fontSize: '25px' }}/>
                        </button>
                    </td>
              </tr> 
            ))
