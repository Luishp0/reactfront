...
function Actualizar() {
      ...
      useEffect(() => {
        const getDesaparecidoById = async () => {
          try {
            ...
            // Realiza la transformación solo si la URL comienza con 'uploads/'
            const transformedData = {
              ...res.data,
              urlFoto: res.data.urlFoto.startsWith('uploads/') ? `http://${process.env.REACT_APP_HOSTAPI}/${res.data.urlFoto.replace(/\\/g, '/')}` : res.data.urlFoto,
            };
  ...
  const ResidenteSiChange = (event) => {
    setDesaparecidoData((prevData) => ({
      ...
      residenteNo: false,
      residente: event.target.checked ? 'Si Recurre' : '',
    }));
  };
  const ResidenteNoChange = (event) => {
    setDesaparecidoData((prevData) => ({
      ...
      residenteSi: false,
      residente: event.target.checked ? 'No Recurre' : '',
    }));
  };
          ...
          <Modal
            ...
            >
              <Paper className="paper-custom">
                ...
                onClick={() => {
                  if (notaData.descripcion) {
                    storeNotas();
                  } else {
                    Swal.fire({
                      icon: 'error',
                      title: 'Por favor, ingresa una descripción antes de agregar una nota.',
                      showConfirmButton: false,
                      timer: 2000,
                      customClass: {
                        popup: 'swal-modal' // Clase personalizada para aplicar estilos al Swal
                      },
                      willOpen: () => {
                        const modalOverlay = document.querySelector('.modal-custom');
                        if (modalOverlay) {
                          modalOverlay.style.zIndex = 1060; // Asegura que el modal tenga un z-index adecuado
                        }
                      },
                      willClose: () => {
                        const modalOverlay = document.querySelector('.modal-custom');
                        if (modalOverlay) {
                          modalOverlay.style.zIndex = ''; // Restablece el z-index del modal
                        }
                      }
                    });
                  }
    ...
    <div className="container">
      ...
      <br></br><br></br>
        ...
        <FormControlLabel
          control={<Checkbox 
            name="residente"        
            checked={desaparecidoData.residente === 'Si Recurre' ? true : false} 
        ...
        <FormControlLabel style={{ marginRight: '30px' }}
          control={<Checkbox 
            name="residente" 
            checked={desaparecidoData.residente === 'No Recurre' ? true : false} 
